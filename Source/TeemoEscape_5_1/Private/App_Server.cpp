...
{
	//teemo_game_instance = Cast<UTeemoGameInstance>(GetWorld()->GetGameInstance());
	UE_LOG(LogTemp, Warning, TEXT("[UApp_Server]"));
}
...
void UApp_Server::OnReceive(const UINT32 client_index, const UINT32 size, char* P_recv_data)
{
	...
	// P_recv_data : WSARecv()내에 사용되는 버퍼 주소
...
void UApp_Server::Run(const UINT32 max_client)
{
	...
	{
		P_packet_manager_ = Cast<UTeemoGameInstance>(GetWorld()->GetGameInstance());
	}
